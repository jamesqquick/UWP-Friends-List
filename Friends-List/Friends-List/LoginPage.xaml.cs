...
namespace Friends_List
{
    ...
    public sealed partial class LoginPage : Page
    {
		...
		{
			if (await AzureUserHelper.CheckLoggedIn())
			{
		...
		private async void LoginButton_Click(object sender, RoutedEventArgs e)
		{
			...
			else
			{
				string response = "";
				if (isLoggingin)
					response = await AzureUserHelper.LoginAsync(UsernameText.Text, PasswordText.Password);
				else
				{
					if (!PasswordText.Password.Equals(RePasswordText.Password))
					{
						...
						response = await AzureUserHelper.SignUpAsync(UsernameText.Text, PasswordText.Password, EmailText.Text);
				}
		...
		public void NavigateHome()
		{
			...
			this.Frame.Navigate(typeof(FriendsList));
		}
